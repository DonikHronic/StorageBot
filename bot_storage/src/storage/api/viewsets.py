...
def change_storage(request):
    ...
    if action not in ChangeHistory.Action.names:
        return Response(['Invalid action. Must be ADD or USED'], status=status.HTTP_400_BAD_REQUEST)

    if count:
        count = int(count)
    else:
        return Response(['Count parameter is required'], status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        if action in ChangeHistory.Action.ADD:
            ...
            return Response(['Your count could not be more then product count'], status=status.HTTP_400_BAD_REQUEST)
        else:
            product.count -= count
