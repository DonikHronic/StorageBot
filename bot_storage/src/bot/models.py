...
class Employee(models.Model):
    ...
    user = models.OneToOneField(
        BaseUser, on_delete=models.CASCADE, related_name="employee"
    )
...
class Client(models.Model):
    ...
    user = models.OneToOneField(
        BaseUser, on_delete=models.CASCADE, related_name="client"
    )
...
class Cart(models.Model):
    ...
    items = models.ManyToManyField(
        Product, verbose_name="Продукты", through="CartItems"
    )
...
class Ticket(models.Model):
    ...
    status = models.CharField(
        "Статус", choices=Statuses.choices, default=Statuses.IN_PROCESS, max_length=25
    )
